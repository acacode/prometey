...
import { createElement, createPrometeyElement } from './src/element'
...
Prometey.connect = (...trees) => {
  if (trees[0].constructor.name === 'Function') {
    ...
    const prometeyElementsTree = createPrometeyElement(component.render())
    ...
    for (let x = 0; x < trees.length; x++) {
      const prometeyElementsTree = createPrometeyElement(trees[x])
      // createDOMElements(prometeyElementsTree)
      console.log('prometeyElementsTree', prometeyElementsTree)
